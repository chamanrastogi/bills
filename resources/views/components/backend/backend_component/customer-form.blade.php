...
<x-form.form :route="$isEdit ? route('customers.update', $customer->id) : route('customers.store')" :method="$isEdit ? 'PUT' : 'POST'" :isEdit="$isEdit" enctype="multipart/form-data"
    ...
    <div class="row">
        ...
        <div class="col-4">
            <div class="mb-3">
                <x-form.input-label for="adhar_no" value="Adhar No" />
                <x-form.text-input type="text" name="adhar_no" :value="$customer->adhar_no ?? ''" placeholder="Opening Balance" />
    ...
    <div class="row mb-3">
        <div class="col-12">
            ...
            <x-form.textarea name="address" rows="2"
                placeholder="Address">{{ $customer->address ?? '' }}</x-form.textarea>
