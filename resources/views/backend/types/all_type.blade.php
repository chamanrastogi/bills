                                ...
                                @foreach ($types as $type)
                                    <tr class="type-{{ $type->id }}">
                                        <td style="width:1%"><span class="form-check form-check-primary"><input
                                                    class="form-check-input mixed_child " value="{{ $type->id }}"
                                        ...
                                        <td class="text-center">
                                            <button type="button"
                                                onClick="statusFunction({{ $type->id }},'Type')"
                                                class="shadow-none badge badge-light-{{ $type->status == 1 ? 'danger' : 'success' }} warning changestatus{{ $type->id }}  bs-tooltip"
                                                data-toggle="tooltip" data-placement="top" title="Status"
                                                data-original-title="Status">{{ $type->status == 1 ? 'Deactive' : 'Active' }}</button>
                                        ...
                                        <td class="text-center">
                                            <div class="action-btns">
                                                ...
                                                <a href="javascript:void(0)"
                                                    onClick="deleteFunction({{ $type->id }},'Type')"
                                                    class="action-btn btn-edit bs-tooltip me-2 delete{{ $type->id }}"
                        ...
                        @if ($types->count() != 0)
                            <div class="ms-3">
                               <div class="form-check form-check-primary form-check-inline">
                                ...
                                <button id="deleteall" onClick="deleteAllFunction('Type')"
                ...
                if (checkedValues.length === 0) {
                    ...
                    checkboxes.forEach(function(checkbox) {
                        ...
                        if (checkbox.checked) {
                            // Add the value to the array
                            checkedValues.push(checkbox.value);
                            var elems = document.querySelector('.type-' + checkbox.value);
                            elems.remove();
                        }
                    });
                    // console.log("Checked Checkbox Values: ", checkedValues);
                    var crf = '{{ csrf_token() }}';
                    $.post("{{ route('type.delete') }}", {
                        _token: crf,
                        id: checkedValues,
                        table: table
                    }, function(data) {
                ...
                document.querySelector('.delete' + id).addEventListener('click', function() {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            var elems = document.querySelector('.type-' + id);
                            elems.remove();
                            var crf = '{{ csrf_token() }}';
                            $.post("{{ route('type.delete') }}", {
                                _token: crf,
